...
let
	...
	initialPositionTank = () => {
		...
		if (randomCoord === 1) {
			...
			init.y = getRandomInt(10, 600); //getRandomInt(300, 600);
...
class GameServer {
	...
	// 			init.x = getRandomInt(40, 1400);
	// 			init.y = getRandomInt(20, 600);
	// 	next: for (let r = 0; r < this.mapArray.length; r++) {
	// 		for (let c = 0; c < this.mapArray[r].length; c++) {
	// 			let wallX = ( c *(this.size / 2) ),
	// 					wallY = ( r *(this.size / 2) ),
	// 					paddingX = 25,
	...
	// 		this.mapArray[r].forEach((cell, c) => {
	// 			let coordinatesWall = {};
	// 			coordinatesWall.x = ( c * (this.size / 2) );
	// 			coordinatesWall.y = ( r * (this.size / 2) );
	// 			if (this.mapArray[r][c] === 1 || this.mapArray[r][c] === 2) {
	// 				obstacleCoordinates.push(coordinatesWall);
	// 			}
	// 		});
	// 	});
